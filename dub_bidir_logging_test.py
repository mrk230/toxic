...
from utils.misc import lower_case, normalize_curses


model_name = "Dub_BiDirLSTM_glove6b200d_reg_logged_CURSENORMED_FULL"
...
# Text preprocessing
# making all lower cased
# train['comment_text'] = train.apply(lower_case, axis=1)
# normalizing curses
train['comment_text'] = train.apply(normalize_curses, axis=1)
...
X_train, X_val, y_train, y_val = train_test_split(train, y, test_size = 0.02,
                                                 random_state=42)
...
x = Bidirectional(LSTM(120, return_sequences=True, name='lstm_layer',
...
# train everything
roc2 = ROC_Eval_Full(X_val, y_val)
...
hist2 = model.fit(X_t,y_train, batch_size=batch_size, epochs=epochs2,
          shuffle=True, callbacks=[roc2] , validation_data=(X_val, y_val))
...
plt.legend(['train_loss', 'val_loss','val_auc'], prop=fontP)
...
sub['comment_text'] = sub.apply(normalize_curses, axis=1)
